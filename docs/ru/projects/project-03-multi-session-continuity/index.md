...
Добавьте агенту контроль скоупа и проверочные шлюзы. Реализуйте чанкование транскриптов, извлечение метаданных, отображение прогресса segmentation и debrief report-флоу с цитатами. Используйте `feature_list.json` для отслеживания статуса фич — по одной фиче за раз, никакого «pass» без доказательств верификации.
