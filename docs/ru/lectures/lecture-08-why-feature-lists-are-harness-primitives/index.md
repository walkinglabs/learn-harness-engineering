...
# Лекция 08. Списки фич как harness primitives для Interview Debrief apps
...
```mermaid
flowchart LR
    Feature["Одна строка фичи"] --> Behavior["Поведение<br/>например: POST /report/items возвращает 201"]
...
Каждая запись фичи должна быть масштаба «можно завершить за одну сессию». Слишком широко — не закончится; слишком узко — растёт накладной расход на управление. «Пользователь может добавить товар в корзину» — хорошая гранулярность. «Реализовать корзину» — слишком широко. «Создать поле name в модели Report» — слишком узко. Как нарезать стейк — не цельным куском и не фаршем.

## Пример из реальной практики

Interview Debrief Coach с 10 фичами. Сравниваются два подхода к учёту:
