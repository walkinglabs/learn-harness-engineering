...
You ask an agent to build an interview debrief app. After it finishes, it tells you "done." You look at the code — transcript uploadentication works, but the training plan button in the interview debrief report does nothing, and the training plan flow isn't connected at all. The problem: you never told it what "done" means, so it used its own standard — "I wrote a lot of code and it looks fairly complete."
...
Neither Claude Code nor Codex automatically knows what you mean by "done." You say "add a interview debrief report feature," and the model's interpretation might be "write a Report component and an addToReport method." But you meant "the user can browse products, add to report, and complete training plan end-to-end." This understanding gap persists without a feature list. The agent uses its own implicit standard — usually "the code has no obvious syntax errors." What you need is end-to-end behavioral verification. Like asking a friend to buy you fruit — you say "get some fruit" and they come back with lemons. Their fruit and your fruit are not the same fruit.

Look at this common progress note:

```
Did transcript upload, interview debrief report mostly done, still need training plans
```
Can a new agent session answer these questions from this note? What does "mostly done" mean? Which tests did the report pass? What's blocking training plans? The answer to all is "nobody knows." Like telling your doctor "my stomach hurts, been okay lately" — what medicine can they prescribe?
...
```mermaid
flowchart LR
    Feature["One feature row"] --> Behavior["Behavior<br/>for example: POST /report/items returns 201"]
...
- **Single source of truth**: All information about "what needs to be done" must derive from one feature list. No contradictions between the feature list and debrief history.
...
Each feature item should be scoped to "completable in one session." Too broad and it won't finish; too narrow and the management overhead grows. "User can generate a transcript-backed report" is good granularity. "Implement the interview debrief report" is too broad. "Create the expanded filler-word counter on the report model" is too narrow. Like cutting a steak — not the whole piece, and not ground meat.

## Real-World Case

An interview debrief coach with 10 features. Two tracking approaches compared:

**Memo mode**: Agent uses unstructured notes. After 3 sessions, notes become "did transcript upload and session list, interview debrief report mostly done but has bugs, training plans not started." New session needs 20 minutes to infer state, ultimately re-implements completed features. Like your shopping list saying "milk, bread, and that thing" — at the store, you still don't know what to buy.
