...
에이전트가 구현과 검증을 동시에 담당하면 자신의 오류를 스스로 발견하기 어렵습니다. 독립된 평가자 역할을 분리하면 환각(hallucination)과 섣부른 완료 선언을 방지하고, 근거 기반(grounded) debrief report의 정확도를 높일 수 있습니다.
...
자기 검증(self-verification) + 근거 기반 debrief report + 증거 기반 완료(evidence-based completion)
