...
此项目 05 变体展示了用于 `ConversationHistory` 的规划器加生成器加评估器模式。它包含已完成的代码、冲刺契约和评估器量规，但不包含完整的综合项目 harness 文件。
...
1. 用 `pwd` 确认工作目录。
2. 阅读 `sprint-contract.md`，了解范围和验收标准。
3. 阅读 `evaluator-rubric.md`，了解最终质量证据。
4. 阅读 `docs/ARCHITECTURE.md`，了解 Electron 层级边界。
5. 阅读 `clean-state-checklist.md`。
6. 若缺少依赖，运行 `npm install`。
7. 运行 `npm run check`。
8. 运行 `bash scripts/check-architecture.sh`。
...
- `sprint-contract.md`：规划器范围、验收标准和冲刺日志
...
- 目标行为已实现
- 所需验证已实际运行
- 证据已记录在 `sprint-contract.md`、`evaluator-rubric.md` 或最终摘要中
- 仓库可从标准启动路径重新开始工作
