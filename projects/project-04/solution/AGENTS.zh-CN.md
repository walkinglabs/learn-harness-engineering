...
本仓库专为长期运行的编码智能体工作而设计。目标不是最大化原始代码产出，而是让下一次会话无需猜测即可继续工作。
...
4. 若缺少依赖，运行 `npm install`。
5. 运行 `npm run check`。
6. 运行 `bash scripts/check-architecture.sh`。
...
项目 04 有意采用比综合项目更精简的 harness。本解决方案中的必需工件为：
...
不要假定本项目存在 `feature_list.json`、`claude-progress.md`、`init.sh` 或 `session-handoff.md`。这些工件会在其他项目阶段引入。
